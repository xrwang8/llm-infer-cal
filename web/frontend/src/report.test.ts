...
describe('report helpers', () => {
  ...
  it('omits inactive optimization details instead of sending stale defaults', () => {
    const payload = buildEvaluatePayload({
      ...
      target_concurrent_requests: '',
      speculative_draft_model_id: ' stale/draft ',
      speculative_extra_weight_gb: '1.5',
      expert_offloading: false,
      experts_on_gpu: '16',
      cpu_offload_gb: '',
      llm_review: false,
    ...
    expect(payload).toMatchObject({ speculative_enabled: false, expert_offloading: false });
    expect(payload).not.toHaveProperty('speculative_mode');
    expect(payload).not.toHaveProperty('speculative_num_draft_tokens');
    expect(payload).not.toHaveProperty('speculative_draft_model_id');
    expect(payload).not.toHaveProperty('speculative_extra_weight_gb');
    expect(payload).not.toHaveProperty('experts_on_gpu');
  });

  it('passes MTP speculative mode without inventing a draft model', () => {
    const payload = buildEvaluatePayload({
      ...
      target_concurrent_requests: '',
      speculative_draft_model_id: ' should/not/send ',
