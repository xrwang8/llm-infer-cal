...
pub fn render_report_json(report: &EvaluationReport) -> Result<String, serde_json::Error> {
    serde_json::to_string_pretty(&json!({
        ...
        "inference_options": {
            ...
            "speculative_enabled": report.speculative_enabled,
            "speculative_mode": report.speculative_mode.as_str(),
            "speculative_num_draft_tokens": report.speculative_num_draft_tokens,
            ...
            "expert_offloading": report.expert_offloading,
            "experts_on_gpu": report.experts_on_gpu,
            "expert_offload_bytes_per_gpu": report.expert_offload_bytes_per_gpu,
        },
    ...
    json!({
        ...
        "options": fleet.options.iter().map(|option| {
            json!({
                ...
                "main_weight_bytes_before_offload_per_gpu": option.main_weight_bytes_before_offload_per_gpu,
                "main_weight_bytes_per_gpu": option.main_weight_bytes_per_gpu,
                "expert_offload_bytes_per_gpu": option.expert_offload_bytes_per_gpu,
